...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if(isset($booking_id) )      {
        ...
        $db->Update("update form_booking_user set `status_booking` =  ? WHERE `booking_id` = ? ", array(3, $booking_id)  );
        ...
        $description = "Order # ".$booking_id.", Ready to Pickup. Have a nice day!. ".date('y-m-d h:m:s');
        ...
        // notify courier
        $description = "Your Order # ".$booking_id.", is Ready to Pickup. Have a nice day!. ".date('y-m-d h:m:s');
        $db->Insert("INSERT INTO notity_system (`user_id`,`description`) VALUES (?,?)", [
            $data[0]["clients"],
            $description
        ]);
