...
class ActionAdminClass {
	...
	public function get_details_users(){
		try {
            ...
            if($name == 'view_details') {
				...
				$user_type = $data[0]["user_type"] == 2 ? "User" :  $user_type;
	...
	public function get_details_users_shops(){
try {
			extract($_POST);
			// $Verify = $name == "reject" ? 3 : 1;
            ...
            if($name == 'view_details') {
				...
				echo $div;
           		...
           		if($data[0]["user_type"] == 4){
					...
					from users pd
					inner join personal_info pi using (user_id)
					inner join shops cd using(p_info_id)
					WHERE user_id = ? limit 1 ", array($user_id));
					$div2 = 'NO record Found!..';
					if(count($couri_details) > 0) {
						if($name == 'view_resume') {
		           			$div2 = "<img src='".$couri_details[0]["logo"]."' style='max-width: 100%;height: auto;text-align: center;margin:0px auto; display: flex;'/>";
		           			// $div2 = '<embed
							// 	    src="http://infolab.stanford.edu/pub/papers/google.pdf#toolbar=0&navpanes=0&scrollbar=0"
							// 	    type="application/pdf"
							// 	    frameBorder="0"
			            ...
			            if($name == 'view_driver') {
			           		$div2 = "<img src='".$couri_details[0]["permit"]."' width='400' height='400' style='max-width: 100%;height: auto;text-align: center;margin:0px auto; display: flex;'/><hr>";
			           		$div2 .= "Summary: ".$couri_details[0]["descriptions"];
			           	}
			           	$div = '<div id="parcel_details"><div class="modal-body">'.$div2.'</div><div class="modal-footer">
						<button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
						</div></div>';
