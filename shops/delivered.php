                                        ...
                                        <tbody>
                                            <?php
                                                $user_to_verify = $db->select("SELECT *, CONCAT(last_name, ', ', first_name ) fullname FROM form_booking_user 
                                                    ...
                                                    where status_booking = 12  and shop_id in (select shop_id from shops where p_info_id = (select p_info_id from personal_info where user_id = ?) ) " , array($_SESSION["user_id"]) );
